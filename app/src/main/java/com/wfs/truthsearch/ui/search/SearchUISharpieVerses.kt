...
@Composable
fun SharpieItemContent(
    result: List<Triple<String, String, String>>, // Full list of results
    onVerseClick: (String) -> Unit
) {
    val permanentMarker = FontFamily(Font(R.font.permanent_marker, FontWeight.Normal))

    result.forEach { (verseId, friendlyVerse, text) ->
        val annotatedLinkString = buildAnnotatedString {
            // Style for the verse reference
            pushStringAnnotation(tag = "verse", annotation = verseId)
            withStyle(
                style = TextStyle(
                    fontFamily = permanentMarker,
                    fontSize = 24.sp,
